...
// // //   // طباعة التواريخ في الكونسول للتأكد من صحتها
...
// //   // تحقق من أن التاريخ المُدخل يقع ضمن الفترة المتاحة
// //   const validateDateRange = (date) => {
// //     if (!date) return "يرجى اختيار التاريخ";
// //     const selectedDate = new Date(date);
// //     if (selectedDate < startAvailableDate || selectedDate > endAvailableDate) {
// //       return "التاريخ محجوز";
// //     }
// //     return true;
// //   };
...
// //   // تحقق من أن تاريخ النهاية أكبر من أو يساوي تاريخ البداية
// //   const validateEndDate = (endDate) => {
// //     if (!startDate) return "يرجى اختيار تاريخ البداية أولاً";
// //     const startDateValue = new Date(startDate);
// //     const endDateValue = new Date(endDate);
// //     if (endDateValue < startDateValue) {
// //       return "يجب أن يكون تاريخ النهاية بعد أو يساوي تاريخ البداية";
// //     }
// //     return true;
// //   };
...
// //   // التحقق من أن التاريخ المُدخل يقع ضمن الفترة المتاحة
// //   const validateDateRange = (date) => {
// //     if (!date) return "يرجى اختيار التاريخ";
// //     const selectedDate = new Date(date);
// //     if (isNaN(selectedDate.getTime())) return "تاريخ غير صالح";

// //     // تطبيع التاريخ إلى تنسيق YYYY-MM-DD لضمان مقارنة صحيحة
// //     const selectedDateString = selectedDate.toISOString().split("T")[0];
// //     const startAvailableString = startAvailableDate.toISOString().split("T")[0];
// //     const endAvailableString = endAvailableDate.toISOString().split("T")[0];

// //     if (selectedDateString < startAvailableString || selectedDateString > endAvailableString) {
// //       return "التاريخ محجوز، الرجاء اختيار تاريخ ضمن الفترة المتاحة";
// //     }
// //     return true;
// //   };
...
// //         <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
// //           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
// //             <input type="date" {...register("startDate", { required: "Start date is required", validate: { validDate: (value) => validateDateRange(value) } })} className="p-4 border border-gray-300 rounded-lg w-full bg-gray-50 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
// //             {errors.startDate && <p className="text-red-500 text-sm">{errors.startDate.message}</p>}

// //             <input type="date" {...register("endDate", { required: "End date is required", validate: { validDate: (value) => validateDateRange(value), validEndDate: (value) => validateEndDate(value) } })} className="p-4 border border-gray-300 rounded-lg w-full bg-gray-50 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
...
// //           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
// //             <input type="date" {...register("startDate", { required: "Start date is required", validate: { validDate: (value) => validateDateRange(value) } })} className="p-4 border rounded-lg w-full" />
// //             <input type="date" {...register("endDate", { required: "End date is required", validate: { validDate: (value) => validateDateRange(value), validEndDate: (value) => validateEndDate(value) } })} className="p-4 border rounded-lg w-full" />
// //           </div>
// //           <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-lg text-lg font-semibold hover:bg-blue-700 transition-all">Confirm Booking</button>
// //         </form>
// //         <ToastContainer />
...
//     set(formRef, data)
//       .then(() => {
//         toast.success("تم حفظ البيانات بنجاح!"); // استخدام toast.success بدلاً من alert
//         onClose();
//       })
//       .catch((error) => {
//         toast.error("خطأ في حفظ البيانات: " + error.message); // استخدام toast.error بدلاً من alert
//       });
//   };

//   return (
//     <>
//       {/* إضافة ToastContainer لعرض التنبيهات */}
//       <ToastContainer
//         position="top-right"
//         autoClose={3000}
...
//           <p className="text-lg text-gray-600 text-center mb-6">Please provide your details to complete the booking</p>
//           <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
//             <input type="text" {...register("fullName", { required: "Full name is required" })} placeholder="Full Name" className="p-4 border rounded-lg w-full" onBlur={(e) => e.target.reportValidity()} />
//             <input type="text" {...register("idNumber", { required: "ID Number is required" })} placeholder="ID Number" className="p-4 border rounded-lg w-full" onBlur={(e) => e.target.reportValidity()} />
//             <input type="number" {...register("numGuests", { required: "Number of guests is required" })} placeholder="Number of Guests" className="p-4 border rounded-lg w-full" onBlur={(e) => e.target.reportValidity()} />
//             <input type="text" {...register("phoneNumber", { required: "Phone Number is required" })} placeholder="Phone Number" className="p-4 border rounded-lg w-full" onBlur={(e) => e.target.reportValidity()} />
//             <input type="text" {...register("address", { required: "Address is required" })} placeholder="Address" className="p-4 border rounded-lg w-full" onBlur={(e) => e.target.reportValidity()} />
//             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
...
const BookNowPopup = ({ productId, availableDates, onClose }) => {
  ...
  const validateDateRange = (date) => {
    ...
    if (selectedDate < startAvailableDate || selectedDate > endAvailableDate) {
      return "The selected date is not available. Please choose a date within the available range.";
    }
    return true;
  };
  ...
  const onSubmit = (data) => {
    ...
    set(formRef, data)
      .then(() => {
        toast.success("Data saved successfully!"); // Success notification
        onClose();
      })
      .catch((error) => {
        toast.error("Error saving data: " + error.message); // Error notification
      });
  };
  ...
  return (
    <>
      ...
      <ToastContainer
        ...
        newestOnTop={false}
        closeOnClick
        rtl={false} // Set to false for English
        pauseOnFocusLoss
        draggable
        pauseOnHover
      />
        ...
        <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-3xl relative">
          ...
          <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
            ...
            <input
              ...
              onBlur={(e) => {
                if (errors.fullName) {
                  toast.error(errors.fullName.message); // Show error toast
                }
              }}
            />
            ...
            <input
              ...
              onBlur={(e) => {
                if (errors.idNumber) {
                  toast.error(errors.idNumber.message); // Show error toast
                }
              }}
            />
            ...
            <input
              ...
              placeholder="Number of Guests"
              className={`p-4 border rounded-lg w-full ${
                errors.numGuests ? "border-red-500" : "border-gray-300"
              }`}
              onBlur={(e) => {
                if (errors.numGuests) {
                  toast.error(errors.numGuests.message); // Show error toast
                }
              }}
            />
            ...
            <input
              ...
              onBlur={(e) => {
                if (errors.phoneNumber) {
                  toast.error(errors.phoneNumber.message); // Show error toast
                }
              }}
            />
            ...
            <input
              ...
              onBlur={(e) => {
                if (errors.address) {
                  toast.error(errors.address.message); // Show error toast
                }
              }}
            />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <input
                ...
                {...register("startDate", {
                  required: "Start date is required",
                  validate: { validDate: validateDateRange },
                })}
                className={`p-4 border rounded-lg w-full ${
                ...
                onBlur={(e) => {
                  if (errors.startDate) {
                    toast.error(errors.startDate.message); // Show error toast
                  }
                }}
              />
              <input
                ...
                {...register("endDate", {
                  required: "End date is required",
                  validate: { validDate: validateDateRange, validEndDate: validateEndDate },
                })}
                className={`p-4 border rounded-lg w-full ${
                ...
                onBlur={(e) => {
                  if (errors.endDate) {
                    toast.error(errors.endDate.message); // Show error toast
                  }
                }}
              />
            </div>
